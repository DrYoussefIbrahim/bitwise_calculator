...
void TWI_Setup(void)
{
	...
	//F_SCL = F_CPU / (16 + 2 * TWBR0 * PRESCALER)
	//TWBR0 = ((F_CPU / F_SCL) - 16) / (2 * PRESCALER)
...
uint8_t TWI_BeginTransmission(void)
{
	...
	while ((!BitCheck(TWCR0, TWINT)) && (i2c_count < i2c_timeout)){
		i2c_count++;
	}
...
void TWI_EndTransmission(void)
{
	...
	while((BitCheck(TWCR0, TWSTO)) && (i2c_count < i2c_timeout)){
		i2c_count++;
	}
}
